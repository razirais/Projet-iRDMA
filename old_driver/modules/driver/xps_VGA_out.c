...
static ssize_t xps_VGA_out_read(
				struct file *filep,
				char *buf,
				size_t count,
				loff_t *offset) 
...
static int xps_VGA_out_open(struct inode *inode, struct file *filep) 
{
  ...
  printk("vga_out: open\n");
  vga_out=container_of(inode->i_cdev,  struct Xps_VGA_out_device,  cdev);
...
static int vga_out_setup(struct Xps_VGA_out_device *vga_out) {
  ...
  if (port) {
    ...
    vga_out->iomem_res = request_mem_region(vga_out->hw_phy_base, 
			     vga_out->hw_length,
			     "vga_out"
			     );
    printk("<1>vga_out: 0x%x (hw_phy) reserved!\n", vga_out->hw_phy_base);
  }
  vga_out->hw_base= ioremap(vga_out->hw_phy_base, vga_out->hw_length);
  ...
  printk(
	 ...
	 vga_out->hw_base, vga_out->it_ipif,vga_out->it, vga_out->control
         );
...
static void __devexit vga_out_teardown( struct Xps_VGA_out_device *vga_out)
{
  ...
  release_mem_region((unsigned int)vga_out->hw_phy_base,vga_out->hw_length);
  free_irq(vga_out->irq, vga_out);
  printk("teardown OK\n");
}
static void vga_out_setup_cdev(struct Xps_VGA_out_device *vga_out, int index)
{
  ...
  if (err)
    printk(KERN_NOTICE " Error %d adding vga_out%d",err,index);
}
...
static int __devinit vga_out_probe(struct platform_device *dev)
{
  ...
  return vga_out_alloc(&dev->dev, id,
...
static inline int __init vga_out_of_register(void) { return 0; }
static inline void __exit vga_out_of_unregister(void) { }
#endif /* CONFIG_OF */
...
static int __init vga_out_init(void)
{
  ...
  if(vga_out_major!=0){
    ...
    rc = register_chrdev_region(dev,vga_out_nr_devs,"vga-out");
  }else{
    rc = alloc_chrdev_region(&dev,vga_out_minor,vga_out_nr_devs,"vga-out");
