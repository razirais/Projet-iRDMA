...
static ssize_t xps_GPIO_XUP_read(struct file *filep,
...
static int xps_GPIO_XUP_open(struct inode *inode, struct file *filep)
{
	...
	gpio_xup =
	    container_of(inode->i_cdev, struct Xps_GPIO_XUP_device, cdev);
...
static int gpio_xup_setup(struct Xps_GPIO_XUP_device *gpio_xup)
{
	...
	port =
	    check_mem_region(gpio_xup->hw_phy_base, gpio_xup->hw_length);
	if (port) {
		...
		request_mem_region(gpio_xup->hw_phy_base,
				   gpio_xup->hw_length, "gpio_xup");
		printk("gpio_xup: 0x%x (hw_phy) reserved!\n",
		       gpio_xup->hw_phy_base);
	}
	gpio_xup->hw_base =
	    ioremap(gpio_xup->hw_phy_base, gpio_xup->hw_length);
...
static void __devexit gpio_xup_teardown(struct Xps_GPIO_XUP_device
					*gpio_xup)
{
	...
	release_mem_region((unsigned int) gpio_xup->hw_phy_base,
			   gpio_xup->hw_length);
	if (gpio_xup->irq_present)
...
static void gpio_xup_setup_cdev(struct Xps_GPIO_XUP_device *gpio_xup,
				int index)
{
	...
	if (err)
		printk(KERN_NOTICE " Error %d adding gpio_xup%d", err,
		       index);
}


static int __devinit gpio_xup_alloc(struct device *dev, int id,
				    resource_size_t hw_phy_base,
				    resource_size_t hw_length, int is_in,
				    int is_out, int irq_present, int irq)
{
	struct Xps_GPIO_XUP_device *gpio_xup;
	int rc;
...
static int __init gpio_xup_init(void)
{
	...
	if (gpio_xup_major != 0) {
		...
		rc = register_chrdev_region(dev, gpio_xup_nr_devs,
					    "xps-gpio");
	} else {
		rc = alloc_chrdev_region(&dev, gpio_xup_minor,
					 gpio_xup_nr_devs, "xps-gpio");
